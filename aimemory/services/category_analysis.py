...
FINE_GRAINED_CATEGORY_GUIDE = (
    "细分类优先级："
    "回答风格=中文优先、语气、称呼、自然收尾、回复方式；"
    "角色设定=AI角色身份、性格、视觉核心设定；"
    "角色关系=用户与AI、AI之间的关系、边界、称呼归属；"
    "群聊规则=QQ群、@触发、例外群、非主人参与者回应规则；"
    "游戏资料=命运2/Destiny 2、游戏任务、装备、通行证、仓库、版本系统；"
    "出图偏好=长期画风、角色外观、黑丝/服装/视觉偏好；"
    "出图流程=Grok/Codex双路出图、图片发送、生成脚本、安全改写流程；"
    "技术资料=账号、接口地址、脚本路径、文件位置、状态验证、固定配置；"
    "故障排查=登录态失效、连接失败、进程崩溃、报错处理；"
    "自动化任务=heartbeat、自动拉起、supervisor、定时/守护任务；"
    "工作流程=部署诊断、操作步骤、项目处理流程；"
    "生活偏好=饮食、作息、日常偏好；"
    "其它=确实无法归类的兜底。"
)
LOCAL_DOMAIN_HINTS = (
    "本地领域提示：命运2/Destiny 2 是游戏，不是技术系统；"
    "相关的猎杀通行证、装备、仓库容量、版本同步、游戏任务、赛季系统、角色配装应归入游戏资料。"
    "绯夜、鸦羽绯夜、月见绫音、绫音、鸦羽是当前用户的 AI 角色/人物关系语境，不是游戏名；"
    "询问“记得绯夜吗、绫音是谁、她们关系、主人/老婆/特殊的人”等应归入角色关系或角色设定，不要归入游戏资料。"
)
    ...
    return [
        {
            ...
            "content": (
                ...
                "分类应细到能减少误召回，不要把所有技术、娱乐或角色内容塞进一个大类。"
                f"{FINE_GRAINED_CATEGORY_GUIDE}"
                "不要把一次性关键词、错别字、工具名、普通名词或用户临时请求直接当分类。"
